...
contract SafeUtils is VaultStorage {
    ...
    event safeUtilsResponse(address feeAddress, string message);

    modifier onlySafeMinter() {
        require(
            msg.sender == IAPContract(APContract).safeMinter(),
            "Only safe minter"
        );
        _;
    }
    ...
    function safeCleanUp(address[] memory cleanUpList) public onlySafeMinter {
    ...
    function paybackExecutor(
        ...
        for (uint256 i = 0; i < gasCost.length; i++) {
            ...
            if (gasToken[i] == eth) {
                ...
                (bool success, ) = to.call{value: gasCost[i]}("");
    ...
    function managementFeeCleanUp(address _tokenAddress) public onlySafeMinter {
    ...
    function processEther(uint256 _etherAmount, address _beneficiary) internal {
        ...
        // to.transfer replaced here
        (bool success, ) = to.call{value: _etherAmount}("");
        require(success, "Transfer failed.");
    }
