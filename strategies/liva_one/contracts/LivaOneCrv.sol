...
contract LivaOneCrv is ERC20 {
    ...
    function addProtocol(address _protocol) external onlyOwner {
        ...
        protocolIndex[_protocol] = protocolList.length;
        protocolList.push(_protocol);
    }

    /// @dev Function to remove protocols from strategy.
    /// @param _protocol Address of the yearn protocol.
    function removeProtocol(address _protocol) external onlyOwner {
        ...
        uint256 index = protocolIndex[_protocol];
        uint256 lastIndex = protocolList.length - 1;
        delete protocolList[index];
        protocolIndex[protocolList[lastIndex]] = index;
        protocolList[index] = protocolList[lastIndex];
        protocolList.pop();
    }

    constructor(address _APContract, address[] memory _protocols)
        ERC20("LVAONE", "LIVA ONE")
    {
        APContract = _APContract;
        for (uint256 i = 0; i < _protocols.length; i++) {
            protocols[_protocols[i]] = true;
            protocolIndex[_protocols[i]] = protocolList.length;
    ...
    function inCaseTokenGetsStuck(address _tokenAddres) external onlyOwner {
