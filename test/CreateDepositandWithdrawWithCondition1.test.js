...
contract("Should create a vault with condition1, test vault functions and deposit 10 different tokens to it", async (accounts) => {
    ...
    it("Should change yieldster god and enable emergency break", async () => {
            
        await apContract.setYieldsterGOD(accounts[2])
        await testVault.toggleEmergencyBreak({from:accounts[2]})
    ...
    it("Should try to set deposit & withdrawal asset enable and disable asset in same call & fail", async () => {
        await expectRevert(
            testVault.setVaultAssets([tokens[3]],[tokens[3]],[tokens[3]],[tokens[3]]), 
           "unauthorized",
       ) 
    })
    ...
    it("Should direct transfer 10 tokens[0] to vault", async () => {
        let token = await ERC20.at(tokens[0])
    ...
    it("Should direct transfer 1 ether to vault", async () => {
        assert.equal(0, convertUtils.from18((await testVault.getTokenBalance(ether)).toString()))
    ...
    //test cases for withdraw
        it("Should not be able to withdraw 2 vault tokens in tokens[0]", async () => {
        let token = await ERC20.at(tokens[0])
        await expectRevert(
        ...
        it("Should not be able to withdraw 10 vault tokens in tokens[0] from user not having vault token", async () => {
        let token = await ERC20.at(tokens[0])
        await expectRevert(
        ...
        it("Should not be able to withdraw 100 vault tokens in ether", async () => {
        ...
        it("Should not be able to withdraw 1000 vault tokens in tokens[2], but enough tokens[2] is not present in vault", async () => {
        let token = await ERC20.at(tokens[2])
        await expectRevert(
        ...
        it("Should be able to set APS by god", async () => {
            god = await apContract.yieldsterGOD();
