...
contract("Should create a vault with condition2, test vault functions and deposit 10 different tokens to it", async (accounts) => {
    ...
    it("Should change yieldster god and enable emergency break", async () => {
     ...
     it("Should set deposit & withdrawal asset where asset exists", async () => {
        try{
    ...
    it("Should set deposit & withdrawal asset where asset do not exists", async () => {
        try{
            await testVault.setVaultAssets(
    ...
    it("Should set deposit & withdrawal asset enable and disable asset in same call", async () => {
        try{
            await testVault.setVaultAssets(
                [tokens[3]],
                [tokens[3]],
     ...
     it("Should set vault smart startegy not approved by aps", async () => {
        try{
    ...
    //test cases for withdraw
        ...
        let token0 = await ERC20.at(tokens[2])

            god = await apContract.yieldsterGOD();
            await apContract.setEmergencyVault(accounts[4]);
            await token0.approve(testVault.address, convertUtils.to18("100"), {
                from: accounts[0]
            })
            try{
                 await testVault.deposit(token0.address, convertUtils.to18("100"), {
                from: accounts[0]
            });
            }catch(err){
               assert.include(err.message, "unauthorized", 
