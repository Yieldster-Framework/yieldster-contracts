...
const ProfitManagementFeeContract = artifacts.require("./managementFeeStrategies/ProfitManagementFee.sol")
...
const { calculateFee, calculateVaultNAVWithoutManagementFee, getManagementFeesAccruedInVaultStrategyBeneficiary } = require("./utils/managementFeeUtils")
...
contract("Should create a vault, test vault functions and deposit 10 different tokens to it", async (accounts) => {
    ...
    let proxyFactory, apContract, tokenFactory, mockPriceModule, yieldsterVaultMasterCopy, testVault, mStorage, feePercentage, profitManagementFee;
    let tokens = [];
    ...
    beforeEach(async function () {
        ...
        profitManagementFee = await ProfitManagementFeeContract.deployed();
    });
    ...
    it("Should add these tokens to priceModule and aps and set yieldsterDAO to accounts 2", async () => {
        ...
        await apContract.setProfitAndPlatformManagementFeeStrategies(profitManagementFee.address, zeroAddr)
    })
    ...
    it(`Should deposit 100 of tokens[0] to the vault and see if fee collected`, async () => {
        ...
        let managementFeesInDAO = await getManagementFeesAccruedInVaultStrategyBeneficiary(mockPriceModule, testVault, tokenSet, apContract, ERC20)
