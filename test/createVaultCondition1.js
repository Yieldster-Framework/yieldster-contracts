...
contract("Yieldster Vault", function (accounts) {
    ...
    describe("Should create a vault with emergency condition 1 - emergency break", async () => {
        let dai, usdc, usdt, crv3,ether;
        let proxyFactory, apContract;
        let yieldsterVaultMasterCopy, testVault;
        ...
        it("changing god and enabling emergency break", async () => {
        ...
        it("direct transfer 10 dai", async () => {
            ...
            assert.equal(100, convertUtils.from18((await dai.balanceOf(testVault.address)).toString()))
            await dai.transfer(testVault.address,convertUtils.to18("10"),{from: accounts[0]});
            assert.equal(100, convertUtils.from18((await testVault.getTokenBalance(dai.address)).toString()))
            assert.equal(110, convertUtils.from18((await dai.balanceOf(testVault.address)).toString()))
        })
