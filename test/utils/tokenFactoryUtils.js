...
exports.mintTokens = async (tokenArr, account) => {

    for (let index = 0; index < tokenArr.length; index++) {
        let token = new web3.eth.Contract(erc20, tokenArr[index], { from: account });
        if (index % 2 === 0)
            token.methods.mintTokens("100000000000000000000000").send();
        else
            token.methods.mintTokens("200000000000000000000000").send();

    }
}


exports.massApprove = async (tokenArr, addr, amount, length, accounts) => {
    for (let index = 0; index < length; index++) {
        let token = new web3.eth.Contract(erc20, tokenArr[index]);
        token.methods.approve(addr, amount).send({ from: accounts[0] });
    }
}

exports.massDeposit = async (tokenArr, yieldsterVault, amount, length, accounts) => {
    // for (let index = 0; index < length; index++) {
    //     console.log(index,":-",tokenArr[index],amount)
    //     await yieldsterVault.deposit(tokenArr[index], amount);
    // }

    let vaultInstance = new web3.eth.Contract(vaultContractABI, yieldsterVault);
    await vaultInstance.methods.deposit(tokenArr, amount).send({ from: accounts[0] });
